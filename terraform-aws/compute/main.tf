...
resource "aws_instance" "mtc_node_id" {
  count         = var.instance_count #1
  instance_type = var.instance_type  #t3.micro - required for k3 resources; use eu-north-1, change regions in the provider
...
resource "aws_lb_target_group_attachment" "mtc_tg_attach" {
  ...
  target_id = aws_instance.mtc_node_id[count.index].id
  port = 8000
}